...
func TestValidator_Authenticate_ConsoleCookieIsAccessToken(t *testing.T) {
	...
	v := auth.NewValidator(testValidatorConfig(), &stubAPIKeyRepo{}, &stubAdminRepo{admins: map[string]*projects.Admin{admin.ID: admin}}, &stubAdminProjectRepo{}, nil, &stubDocDB{}, nil)
...
func TestValidator_Authenticate_APIKeyIsService(t *testing.T) {
	...
	v := auth.NewValidator(testValidatorConfig(), &stubAPIKeyRepo{keys: map[string]*projects.APIKey{hashSecret(secret): key}}, &stubAdminRepo{}, &stubAdminProjectRepo{}, nil, &stubDocDB{}, nil)
