...
// Round3 H5-4：CreateMembership 幂等——同一 team 下同 user / 同（规范化）
// email 的第二次创建（含 pending 重复邀请、accepted 重复）必须 AlreadyExists，
// 且不重复写行、不再 +1 total。
func TestTeams_CreateMembership_Idempotent(t *testing.T) {
	docDB := newFakeDocDB()
	docDB.seed("teams",
		databases.Document{ID: "team-1", Data: map[string]any{"name": "T", "total": int64(1)}},
	)
	docDB.seed("users",
		databases.Document{ID: "u-1", Data: map[string]any{"email": "a@b.c"}},
	)
	docDB.seed("memberships",
		databases.Document{ID: "m-1", Data: map[string]any{"team_id": "team-1", "user_id": "u-1", "email": "a@b.c", "status": teams.StatusAccepted}},
		databases.Document{ID: "m-2", Data: map[string]any{"team_id": "team-1", "user_id": "", "email": "p@x.com", "status": teams.StatusPending}},
	)
	uc := NewTeams(fakeProjectRepo{}, docDB)
	principal := databases.Principal{Roles: []string{"admin"}}

	teamTotal := func() int64 {
		doc, err := docDB.GetDocument(context.Background(), "proj-1", "default", "teams", "team-1", databases.SystemPrincipal)
	...
	t.Run("accepted duplicate by user_id", func(t *testing.T) {
		...
		_, err := uc.CreateMembership(context.Background(), "proj-1", CreateMembershipCommand{
			TeamID: "team-1", UserID: "u-1", Roles: []string{teams.RoleMember}, Status: teams.StatusAccepted,
		}, principal)
		...
		require.Equal(t, int64(1), teamTotal(), "total 不得 +1")
	})

	t.Run("duplicate by normalized email resolving same user", func(t *testing.T) {
		before := membershipCount()
		_, err := uc.CreateMembership(context.Background(), "proj-1", CreateMembershipCommand{
			TeamID: "team-1", Email: "A@B.C", Roles: []string{teams.RoleMember}, Status: teams.StatusPending,
		}, principal)
		...
		require.Equal(t, int64(1), teamTotal())
	})

	t.Run("pending duplicate by email", func(t *testing.T) {
		before := membershipCount()
		_, err := uc.CreateMembership(context.Background(), "proj-1", CreateMembershipCommand{
			TeamID: "team-1", Email: "P@X.com", Roles: []string{teams.RoleMember}, Status: teams.StatusPending,
		}, principal)
	...
	t.Run("fresh accepted invite succeeds and increments total", func(t *testing.T) {
		...
		mem, err := uc.CreateMembership(context.Background(), "proj-1", CreateMembershipCommand{
			TeamID: "team-1", UserID: "u-2", Email: "new@x.com", Roles: []string{teams.RoleMember}, Status: teams.StatusAccepted,
		}, principal)
		require.NoError(t, err)
		require.Equal(t, "new@x.com", mem.Data["email"], "email 落库为规范化小写")
		require.Equal(t, before+1, membershipCount())
		require.Equal(t, int64(2), teamTotal(), "accepted 成功后 total 才 +1")
	})
}
