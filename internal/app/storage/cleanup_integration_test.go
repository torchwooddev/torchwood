...
func TestCleanupOrphanChunks(t *testing.T) {
	...
	require.NoError(t, objStore.Put(ctx, "Torchwood-files", orphanChunk, bytes.NewReader(make([]byte, 1<<20)), 1<<20, ""))
	require.NoError(t, objStore.SetObjectTime("Torchwood-files", orphanChunk, time.Now().Add(-49*time.Hour)))

	// 旧文件对象（无 /chunks/ 段，不应被清理）。
	oldFileKey := projectID + "/" + bucketID + "/old-file-id"
	require.NoError(t, objStore.Put(ctx, "Torchwood-files", oldFileKey, bytes.NewReader([]byte("x")), 1, ""))
	require.NoError(t, objStore.SetObjectTime("Torchwood-files", oldFileKey, time.Now().Add(-49*time.Hour)))
	...
	// 孤儿分片已删；活跃分片与旧文件对象仍在。
	_, err = objStore.Get(ctx, "Torchwood-files", orphanChunk)
	require.Error(t, err, "孤儿分片应被清理")
	_, err = objStore.Get(ctx, "Torchwood-files", activeChunk)
	require.NoError(t, err, "活跃会话分片不应被清理")
	_, err = objStore.Get(ctx, "Torchwood-files", oldFileKey)
...
func TestDeleteBucket_RemovesOrphanChunks(t *testing.T) {
	...
	_, err = objStore.Get(ctx, "Torchwood-files", chunk)
	require.NoError(t, err)
