...
- 系统资源（users、sessions、files、buckets、groups）和用户动态集合使用 PostgreSQL 动态文档 adapter（schema-per-database：`tw_<project.id>_<database.id>` + `_tenant` + `_perms`）。`project.id` / `database.id` 规则见 `docs/developer/06-databases.md`。
