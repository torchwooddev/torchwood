...
- **权限内核（阶段③，redesign §3.2/§4.3）**：文档 ACE 内嵌 `_acl TEXT[]`（`_perms` 表退役）；**权限判定在 RLS policy（`tw_visible`，业务文档表四条 policy + FORCE，`internal/infra/documentdb/rls_policy.go`），角色经 `SET LOCAL ROLE` + `app.roles` GUC 注入（每请求一事务，`clients.Database.RunInTx`；漏注入 fail-closed）**；sentinel 系统集合（测试面）保留应用层判定 `AllowsDocumentAccess`；`_acl` 写入走同事务 tw_system 第二语句（PG 对修改 SELECT policy 引用列的 UPDATE 以新行复检——自锁语义由此保留）。
