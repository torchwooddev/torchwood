...
| TypeScript SDK | `sdk/typescript` 封装 Client + Server API，适配 Agent 工作流 |

核心功能域：

- **项目管理**：多项目隔离，每个项目对应独立 PostgreSQL schema；
- **用户认证**：邮箱注册/登录、JWT access/refresh、会话 cookie、API Key 认证、OTP/Magic URL/OAuth2/MFA；
- **动态文档数据库**：schema-per-database，集合即真实表，`_tenant` 隔离项目，`_perms` 实现文档级权限；
- **文件存储**：S3/MinIO 兼容对象存储，multipart 上传/下载，文件元数据以动态文档管理；
- **函数执行**：Docker 真实执行器（构建/运行）与异步 worker（`cmd/worker`）；
- **Admin Console**：React SPA，嵌入 Go 二进制，通过 `/console/` 提供管理界面；
- **Server API**：Projects、API Keys、Users、Storage、Databases、Collections、Attributes、Indexes 等管理面 CRUD。
...
├── docs/                               # 设计文档（roadmap、tech-decision、archived/）
...
│   ├── client/v1/  server/v1/  console/v1/  shared/v1/
├── sdk/                                # TypeScript SDK（typescript/）与 demo
