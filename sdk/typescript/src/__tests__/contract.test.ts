...
describe("contract: swagger ↔ TS SDK", () => {
  ...
  it("写方法 HTTP 绑定（method/path/query/body 参数）与 swagger 一致", async () => {
    ...
    cases.push(
      ...
      { side: "server", operationId: "DatabasesService_UpdateDocument", invoke: (h) => server(h).db.updateDocument("app", "members", "d1", { data: { n: 2 }, increment: { cnt: 1 } }) },
      { side: "server", operationId: "DatabasesService_DeleteDocument", invoke: (h) => server(h).db.deleteDocument("app", "members", "d1") },
      ...
      { side: "client", operationId: "AccountService_UpdateAccount", invoke: (h) => client(h).account.updateAccount({ name: "N", email: "e@x.c", password: "pw", old_password: "old" }) },
