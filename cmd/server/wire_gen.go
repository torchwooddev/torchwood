...
func wireBootstrap(app lynx.App) (*boot.Bootstrap, func(), error) {
	...
	groups := server.NewGroups(projectsRepository, documentDB)
	...
	assetsAssets := assets.NewAssets(database, defRepo, holdingRepo, ledgerRepo, eventOutbox, logger, projectsRepository)
	...
	subscriptionsSubscriptions := subscriptions.NewSubscriptions(appConfig, database, planRepo, subscriptionRepo, assetsAssets, orderRepo, registry, adapter, eventOutbox, logger, projectsRepository, providerIndexRepo)
	fulfiller := subscriptions.NewOrderFulfiller(assetsAssets, subscriptionsSubscriptions)
	paymentsPayments := payments2.NewPayments(appConfig, database, orderRepo, callbackEventRepo, fulfillmentRepo, fulfiller, registry, eventOutbox, logger, subscriptionsSubscriptions, projectsRepository, providerIndexRepo)
