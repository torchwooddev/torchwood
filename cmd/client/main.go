...
（服务端 gRPC 仅监听回环，远程使用需走 SSH 隧道或调整 server.grpc.addr）。`,
	...
	root.PersistentFlags().StringVar(&g.apiKey, "api-key", envOr("TORCHWOOD_CLI_API_KEY", ""), "API Key secret（亦可用 TORCHWOOD_CLI_API_KEY 环境变量；health 命令除外必填）")
...
// annotationNoKey 标记命令无需 API key（仅 health 命令，见设计文档 §4.3）。
const annotationNoKey = "torchwood/no-key"

// cmdKeyExempt 判断命令是否豁免 API key 必填校验：health 命令（含子命令）公开可调用。
...
func (g *globalFlags) validate(cmd *cobra.Command, _ []string) error {
	...
	if g.apiKey == "" && !cmdKeyExempt(cmd) {
		return fmt.Errorf("缺少 API key：请通过 --api-key 或 TORCHWOOD_CLI_API_KEY 提供（health 命令除外）")
	}
	return nil
}
